...
class SDCTextField(val node: ServerDrivenNode, val state: MutableMap<String, String>) : Layout {
    ...
    private val placeholder: @Composable () -> Unit? = {
        node.propertyNodes("placeholder").let {
            for (serverDrivenNode in it) {
                SDCLibrary.loadComponent(node = serverDrivenNode, dataState = state)
            }
        }
    ...
    private val leadingIcon: @Composable () -> Unit? = {
        node.propertyNodes("leadingIcon").let {
            for (serverDrivenNode in it) {
                SDCLibrary.loadComponent(node = serverDrivenNode, dataState = state)
            }
        }
    }
//    private val trailingIcon: @Composable () -> Unit? = {
//        node.propertyNodes("trailingIcon").let {
//            for (serverDrivenNode in it) {
//                SDCLibrary.loadComponent(node = serverDrivenNode, dataState = state)
//            }
//        }
